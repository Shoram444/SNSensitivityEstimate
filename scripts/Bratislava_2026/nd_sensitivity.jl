...
if radon_tag == 1
    radon_level = 150/1e6
    set_activity!(background[7], radon_level) # radon to 150 uBq/m3
elseif radon_tag == 2
    radon_level = 2/1e3
    set_activity!(background[7], radon_level) # radon to 2 mBq/m3
elseif radon_tag == 3
    radon_level = 2/1e3
    set_activity!(background[7], radon_level) # radon to 2 mBq/m3
elseif radon_tag == 4
    radon_level = 38.21/1e3
    set_activity!(background[7], radon_level) # radon to 38.
elseif radon_tag == 5
    radon_level = 1.08/1e3
    set_activity!(background[7], radon_level) # radon to 1.08.
end
...
options = Options(;
    ...
    f_tol_rel = 1e-5,
    f_tol_abs = 1e-5,
    time_limit = 10.0*60,#analysisDict[:hours]*60*60.0, #60*60*12.0,
...
if analysisDict[:side] == "both"
    x0 = ( float.( [0, 180, 2700, 3100, 0, 100, 0, 100, 0, 4, 1, 50] ) )
...
begin
    ...
    best_sens = get_sensitivityND(SNparams, α_, PROCESSES, my_roi; approximate="table")
    print(best_sens)
end
...
df = DataFrame(
    signal = signal.isotopeName,
    radon_level = radon_level,
    eff = best_sens.signalEff,
    bkgCounts = best_sens.bkgCounts,
    tHalf = best_sens.tHalf,
    roi = my_roi,
)

CSV.write(scriptsdir("Bratislava_2026/nd_sensitivity.csv"), df, append=true)
