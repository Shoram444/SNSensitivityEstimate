...
f = ROOTFile("/home/maros/Work/Phd/SNSensitivityEstimate/data/data/v2_phase0_Xalbat_300kev/mva.root")
...
begin
    ...
    phi_low, phi_high = 0, 165 # degrees
    E_cut = 300 # keV
    dy_cut = 100 # mm
    dz_cut = 100 # mm
    ...
    begin
        ...
        a= Axis(f[1,1], xlabel = "E1 + E2", ylabel = "counts", title = "data bb phase0", limits = (0, 3500, nothing, nothing))
        ...
        safesave(joinpath(scriptsdir("data/phase0"), "sum_spectrum.png"), f, px_per_unit = 5)
        f
    end
end
...
let
    ...
    m = 6.11
    d_p0  = 2.7740347e6
    n = eff * act * d_p0 * m # number of events in 1 year
    f = Figure()
    a= Axis(
        f[1,1], 
        xlabel = "E1 + E2", 
        ylabel = "counts", 
        title = L"data bb phase0; \\ aquisiton time $\approx$ %$(round(d_p0/3600/24, digits =1)) days", 
    ...
    save(joinpath(scriptsdir("data/phase0"), "phase0_2e_spectrum_2nu.png"), f, px_per_unit = 5)
    f 
end