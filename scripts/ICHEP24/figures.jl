...
with_theme(theme_latexfonts()) do 
    ...
    p1 = lines!(a1,range(0,3000, length(sum_spectrum)) ,sum_spectrum, color = orange, linewidth = 3.5, label= L"standard $2\nu\beta\beta$")
    p2 = lines!(a1,range(0,3000, length(sum_spectrum_RH)) ,sum_spectrum_RH, color = aqua, linewidth = 3.5, label= L"$2\nu\beta\beta$ with $\bar{\nu}_R$")

    p3 = lines!(a2, range(0,180, length(ys)), ys, color = orange, linewidth = 3.5, label = L"standard $2\nu\beta\beta$")
    p4 = lines!(a2, range(0,180, length(ys_RH)), ys_RH, color = aqua, linewidth = 3.5, label = L"$2\nu\beta\beta$ with $\bar{\nu}_R")
