...
data_dir = datadir("data/final_phd/data_no_gamma_alpha_veto")
...
begin
    var_data = :trackLength1
    var_simu = :trackLength1
    binning = (0:50:1500)
    data_cuts = Dict(
        :sumE => (0, 3500),
        :e1 => (250, 5000),
    ...
    simu_h_p1_K40 = get_expected_simu_counts(p1_K40_process, var_simu, 0.12, bins = binning) *1.0
    simu_h_p1_Pa234m = get_expected_simu_counts(p1_Pa234m_process, var_simu, 0.12, bins = binning) *1.0
    simu_h_p1_bb = get_expected_simu_counts(p1_bb_process, var_simu, 0.12, bins = binning) *1.0
    # simu_h_p1_K40_PMT = get_expected_simu_counts(p1_K40_PMT_process, var_simu, 0.12, bins = binning) * 1.0
    simu_h_p1_radon = get_expected_simu_counts(p1_radon_process, var_simu, 0.12, bins = binning) *1.0

    simu_h_p2_K40 = get_expected_simu_counts(p2_K40_process, var_simu, 0.12, bins = binning, timeMeas = p2_duration_seconds) *1.0
    simu_h_p2_Pa234m = get_expected_simu_counts(p2_Pa234m_process, var_simu, 0.12, bins = binning, timeMeas = p2_duration_seconds) *1.0
    simu_h_p2_bb = get_expected_simu_counts(p2_bb_process, var_simu, 0.12, bins = binning, timeMeas = p2_duration_seconds) *1.0
    # simu_h_p2_K40_PMT = get_expected_simu_counts(p2_K40_PMT_process, var_simu, 0.12, bins = binning, timeMeas = p2_duration_seconds) * 1.0
    simu_h_p2_radon = get_expected_simu_counts(p2_radon_process, var_simu, 0.12, bins = binning, timeMeas = p2_duration_seconds) *1.0
    ...
    let 
        ...
        f = Figure(size = (1900, 1200), fontsize = 30)
        a1 = Axis(f[1,1], xlabel = "sumE", ylabel = "counts / 100keV ", title = "phase 2 data vs simu", limits = (binning[1], binning[end], 0, nothing))

        p1 = scatter!(a1, h_data_tot, label = L"data, n = %$(round(integral(h_data_tot))) evts $$", color = :black, markersize = 12)
        errorbars!(a1, h_data_tot, whiskerwidth = 10, color = :black)
        p2 = stephist!(a1, simu_h_p2_K40 , label = L"MC $^{40}$K, a = 58.7 mBq/kg", color = :orange,linewidth =3)
        p3 = stephist!(a1, simu_h_p2_Pa234m , label = L"MC $^{234\text{m}}$Pa, a = %$(Pa_level) mBq/kg", color = :green,linewidth =3)
        p4 = stephist!(a1, simu_h_p2_bb , label = L"MC $2\nu\beta\beta$, $T^{1/2} = 8.69 \times 10^{19}$ yr", color = :blue, linewidth =3)
        p5 = stephist!(a1, simu_h_p2_radon , label = L"MC radon, a1= %$(radon_level2) mBq/$\textrm{m^3}$", color = :purple, linewidth =3)
        # p6 = stephist!(a1, h_flat, label = L"arbitrary flat bkg, n = %$(round(integral(h_flat))) evts $$", color = :cyan, linewidth =3)
        # p6 = stephist!(a1, simu_h_p1_K40_PMT + simu_h_p2_K40_PMT, label = "K40 PMT simu, n = $(round(integral(simu_h_p1_K40_PMT + simu_h_p2_K40_PMT)))", color = :cyan, linewidth =3)
        h_simu_total = simu_h_p2_K40 + simu_h_p2_Pa234m + simu_h_p2_bb + simu_h_p2_radon + h_flat #+ simu_h_p2_K40_PMT
        
        stephist!(a1, h_simu_total, label = L"MC total, n = %$(round(integral(h_simu_total))) evts $$", color = :red, linewidth = 4)
        errorbars!(a1, h_simu_total, whiskerwidth = 10, color = :red)

        axislegend(a1, nbanks = 2, position = :lt)
        ...
        safesave("scripts/phd_final/08_data/figs/PHASE2_sim_vs_data.png", f)
        f
    end
