...
if analysisDict[:side] == "both"
    ...
    ub = float.([80, 180, 3000, 3200, 10, 150, 10, 150, 0, 10, 10, 50])
elseif analysisDict[:side] == "sameSide"
    lb = float.([0, 0, 300, 2000, 0, 50, 0, 50, 0,2, 0, 0, 0, 50 ])
    ub = float.([180, 180, 3000, 3200, 10, 150, 10, 150,  0,2,0, 10, 10, 50])
else 
    lb = float.([0, 0, 300, 2000, 0, 50, 0, 50, 0,1, 0, 0, 0, 50 ])
    ub = float.([180, 180, 3000, 3200, 10, 150, 10, 150,  0,1,0, 10, 10, 50])    
end
...
let 
    ...
    roi_sens = get_sensitivityND(SNparams, α_, PROCESSES, my_roi; approximate="table")
    print(roi_sens)
end
