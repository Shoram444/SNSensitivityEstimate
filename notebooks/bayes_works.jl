...
# ╔═╡ 63c6392d-9cd2-489b-bea3-b5e801ec235f
using FHist, SpecialFunctions

# ╔═╡ 5cf128ae-de71-4b97-97a4-c55738d7f612
mu = rand(truncated(Normal(5.0, 3), 0 ,10))

# ╔═╡ 637b7015-c11a-48a5-9c26-7e531c079b97
data = rand(Normal(mu), 1000)

# ╔═╡ 34dbd872-d99b-4f73-83db-06cb2b22c5c7
n = 1000

# ╔═╡ e8d03336-200b-4686-9608-a4f148617fe0
grid = range(0.001, 10, n)

# ╔═╡ e0a83a95-e351-463b-a54f-b99ee883c625
data_hist = Hist1D(data; binedges = 0:0.1:10)

# ╔═╡ 016842ee-f7ec-4ed4-93b3-f50676cad22a
get_likelihood(data, par) = sum(pdf(Normal(par), data)) 

# ╔═╡ 4a1f7ace-1ffe-4026-a668-ab2b641b8468
sample_likelihood = map(x -> get_likelihood(data,x), grid) 

# ╔═╡ 2d099325-1531-4776-a082-035466e4a033
md"""
λ = $(@bind mup PlutoUI.Slider(0:0.1:10, default=0; show_value=true))

"""

# ╔═╡ a45c2dfd-50d6-4398-95d2-23b0109fca26
prior = pdf.(Normal(mup, 1), grid)

# ╔═╡ dd9aa964-7999-4ae6-a1b6-6e3beefba0f5
plot(grid, prior)

# ╔═╡ f686d8d4-b943-4736-81ae-8e42b32db584
unnormalized_posterior = sample_likelihood .* prior

# ╔═╡ cd39b113-e94d-48e4-9c9e-f87b14a837f7
function compute_log_posterior(data::Vector{<:Real}, mu_grid::Vector{<:Real}, prior::Vector{<:Real}; sigma::Real = 1.0)
    @assert length(mu_grid) == length(prior) "mu_grid and prior must be the same length"

    # Precompute log of prior, adding a small epsilon to avoid log(0)
    log_prior = log.(prior .+ eps())

    # Compute log-likelihood for each mu
    log_likelihood = [
        sum(logpdf(Normal(mu, sigma), x) for x in data)
        for mu in mu_grid
    ]
    ...
    # Stabilize before exponentiating (subtract max)
    log_posterior_max = maximum(log_posterior)
    posterior = exp.(log_posterior .- log_posterior_max)

    # Normalize
    delta = mu_grid[2] - mu_grid[1]
    posterior ./= sum(posterior) * delta
...
begin
	...
	vline!([mu], label="True μ = $(round(mu))", linestyle=:dash, color=:red, linewidth = 4)

	log_posterior = compute_log_posterior(data, collect(grid), prior)
	
	plot!(grid, log_posterior, linewidth = 5, label = "posterior, sample_mu = $(round(grid[argmax(log_posterior)]))")
...
manifest_format = "2.0"
project_hash = "58e64454eca8ea65cf882a5fd9fbf2b8f9928af7"
